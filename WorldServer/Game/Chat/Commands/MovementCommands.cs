...
namespace WorldServer.Game.Chat.Commands
{
    public class MovementCommands : Globals
    {
        ...
        public static void TeleList(string[] args, WorldClass session)
        {
            ...
            if (result.Count == 0)
            {
                var chatMessage = new ChatMessageValues(MessageType.ChatMessageSystem, infoMsg);
                ChatHandler.SendMessage(ref session, chatMessage);
            }
            else
            {
                var chatMessage = new ChatMessageValues(MessageType.ChatMessageSystem, string.Format("{0} match(es) found:", result.Count));
                ChatHandler.SendMessage(ref session, chatMessage);
        ...
        [ChatCommand("listLocs", "Usage: !listLocs [#string] (lists all WorldSafeLocs from DBC file [which contains #string])")]
